...
const client = new DeliverooApi(
    ...
    'http://localhost:8080',
    'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6ImJmMmE1NGZmMTgzIiwibmFtZSI6ImFnZW50MSIsImlhdCI6MTcyMDgwNDI2MH0.IP3S0uVsk97uG6hFxsyXKxEzqu6Ba2vavJ9k2PYOTZg'
)
...
client.onMsg(async (id, name, msg, reply) => {
    ...
    if (myPartnerId == undefined && name == 'agent2'){  // receive partner ID for first time
    ...
    if(name === "agent2"){  // only react on messages from partner
        ...
        while ( true ) {
            ...
            if ( this.intention_queue.length > 0 ) {
                ...
                if(intention.predicate[0] == 'go_pick_up'){
                    ...
                    if (myPartnerId !== undefined && !areIntentionsEqual(currentIntention, lastIntention)){
                        lastIntention.x = x;
                        lastIntention.y = y;

                        let reply = await client.ask(myPartnerId, `${x},${y}` + ',where are you going?');

                        let coordinates = reply.split(",")
                        var other_x = parseFloat(coordinates[0]);
                        var other_y = parseFloat(coordinates[1]);
                        
                        if((distance({x: x, y: y}, {x: other_x, y: other_y})) <= 3){
